...
joan.baseURL = `http://nginx:80/joan/`;

console.log("After setting baseURL:", joan.baseURL);
...
it("should register a user", async () => {
  ...
  const uuid = await joan.createUser(hash, (k) => { keys = k; keysToReturn = k; }, () => { return keys; });
  savedUser.uuid = uuid;
...
it("should delete a user", async () => {
  ...
  const res = await joan.deleteUser(savedUser.uuid);
